...
> **自动化对齐**: `tests_py/generated/test_gt_ads_services_web_e002b.py` — 用例编号 **连续 TC001–TC024**，与 `test_tc001_*` … `test_tc024_*` 一一对应。  
> **覆盖范围（合并自两份文档）**:
>
...
| `GT_ENABLE_PLACEHOLDER_CAROUSEL` | `0` | 设为 `1` 时执行 **可选** TC004 placeholder 轮播 |
| `GT_ENABLE_HOVER_TOOLTIP` | `0` | 设为 `1` 时执行 **可选** TC007 hover tooltip |
...
### TC004（可选）: placeholder 轮播 — `GT_ENABLE_PLACEHOLDER_CAROUSEL=1`

> 对应脚本：`test_tc004_search_placeholder_carousel_changes_optional_e002b`。易波动，**默认不跑**。

#### 执行步骤

1. 设置 `GT_ENABLE_PLACEHOLDER_CAROUSEL=1`
2. 进入 landing
3. 读取第一个 textbox 的 `placeholder`，等待约 **9s**，再读一次

#### 预期结果

- 两次 placeholder **不同**且非空

---

### TC005: `Most popular categories` 区域 **标题**可见
...
### TC006: `All Categories` 在 landing 上可见（点击在 TC008）
...
### TC007: hover 类目卡片出现 tooltip（可选）— `GT_ENABLE_HOVER_TOOLTIP=1`

> 与 placeholder 环境变量 **无关**。

#### 执行步骤

1. 设置 `GT_ENABLE_HOVER_TOOLTIP=1`
2. 进入 landing
3. hover 匹配 `Removal|Cleaner|Builder|Plumber|Electrician` 的任一元素
4. 断言存在 `role=tooltip` 且可见

#### 预期结果

- tooltip 可见

**脚本**: `test_tc007_hover_category_shows_tooltip_optional_e002b`

---

### TC008: 点击 `All Categories` 进入全量 services 结果（URL 断言）
...
### TC010: Discovery more — 三个分区 **标题**可见（不校验卡片数量/对齐）
...
### TC011: Join us — `advertise-with-us` 外链
...
### TC013: SRP Filters — `Location`、`Category` 文案可见
...
### TC014: Distance 下拉 + `Update`（若页面支持）
...
### TC015: SRP 卡片存在 `Request a quote`
...
### TC016: 存在 `GtTestAd` 标记时，该卡片含 `Request a quote`

#### 前置条件

- 页面出现文案 **`GtTestAd`**（否则 skip）

#### 执行步骤

1. 在标记祖先容器内校验 **`Request a quote`**

#### 预期结果

- **`Request a quote` 必校验**；**`Show phone number` 非必**

**脚本**: `test_tc016_gt_ad_card_has_ctas_when_marker_present_e002b`

---

### TC017: Sponsored 区域存在时可点击外链（可选）

#### 执行步骤

1. 进入 SRP
2. 若存在 `Sponsored` 文案，则点击页面首个 `http` 外链（期望新 tab）

#### 预期结果

- 新页面加载；无 Sponsored 则 skip

**脚本**: `test_tc017_srp_sponsored_section_clickable_optional_e002b`

---

### TC018: Pagination — 翻到第 2 页
...
| TC | 说明（docstring 意图） | 脚本函数名 |
| --- | --- | --- |
| TC019 | VIP 标题 Business profile | `test_tc019_vip_business_profile_title_visible_e002b` |
| TC020 | Services and skills | `test_tc020_vip_services_and_skills_visible_e002b` |
| TC021 | Manage skills 卡片 | `test_tc021_vip_manage_skills_card_has_items_e002b` |
| TC022 | More options / Edit set / Delete | `test_tc022_vip_more_options_contains_edit_set_and_delete_e002b` |
| TC023 | Manage ads 入口可见 | `test_tc023_vip_manage_ads_visible_and_has_entry_e002b` |
| TC024 | Manage ads 打开推广流程 | `test_tc024_vip_manage_ads_entry_opens_flow_e002b` |

Preconditions 文档与脚本一致时写作「同 **TC019**（SRP + listing + VIP 模板含 Overview/Services）」。
